...
const Home = () => {
  const { data: newestProducts, loading: newestProductsLoading, error: newestProductsError } = useFetch('/api/public/listings?is_publish=1&limit=5', { cache: 'no-store' });
    ...
    <div className="w-full h-full absolute top-0 left-0 bg-black/[50%]">
      <Container className="!p-4">
        ...
        <div className="w-[90%] mx-auto bg-black/[40%] rounded-md p-4">
          {
            newestProductsLoading ? <Loader className="text-white" /> : newestProductsError ? <Alert type="error" message={newestProductsError} /> : <ProductGrid products={newestProducts} className="overflow-x-auto" />
          }
        </div>
      </Container>
    </div>
